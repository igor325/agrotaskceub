...
1.  **Backend:** Implementar a arquitetura RESTful (RNF07) e o CRUD (Criar, Ler, Atualizar, Deletar) para as entidades principais: `User` (Funcionários) e `Activity` (Tarefas).

2.  **Autenticação:** Criar o sistema de login seguro (RNF02 - JWT) para os `AdminAccount` (Gestores).

3.  **Integrações (Spike/Prova de Conceito):** Estabelecer as conexões iniciais com as APIs do JetBov e do WhatsApp (Meta) para validar a comunicação.

4.  **Frontend (Wireframes):** Desenvolver os protótipos de tela (wireframes) do painel de gestão, conforme definido nos "Próximos Passos" (RNF01, RNF08).
